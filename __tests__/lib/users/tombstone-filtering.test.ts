 ...
 *
 * Guard for #1406: every SQL query that reads `users` (`sql` templates and
 * `.query(` template strings) must either exclude
...
const INTERNAL_FILES: Record<string, string> = {
  ...
  "app/api/routes-f/webhooks-mux-asset/route.ts":
    "provider events must be recorded for every account",
  "app/api/routes-f/webhooks-mux-live/route.ts":
    "provider events must be recorded for every account",
...
function usersQueries(source: string): Array<{ line: number; text: string }> {
  ...
  // `sql` tagged templates and template strings passed to `.query(`.
  const template = /(?:sql|\.query\(\s*)`([\s\S]*?)`/g;
