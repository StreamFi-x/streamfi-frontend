...
describe("socialLinks contract", () => {
  ...
  it("normalises [{url}] written by profile-update-social-links", () => {
    expect(
      prepareSocialLinks([
        { url: "https://www.tiktok.com/@a" },
        { platform: "Website", url: "https://a.io" },
      ])
    ).toEqual({
  ...
  it("rejects wrong primitive types and malformed nested values", () => {
    ...
    expect(() => prepareSocialLinks([{ link: "https://a" }])).toThrow(
      JsonbContractError
    );
...
describe("creator contract", () => {
  ...
  it("accepts the keys other dev routes read and write", () => {
    expect(
      prepareCreator({
        customThumbnailUrl: "https://cdn.example.com/t.png",
        customThumbnailUpdatedAt: "2026-09-25T10:00:00.000Z",
        subscriptionPrice: 4.99,
        subscription_price_usdc: "4.99",
      })
    ).toBeTruthy();
    expect(() => prepareCreator({ subscriptionPrice: -1 })).toThrow(
      JsonbContractError
    );
    expect(() => prepareCreator({ subscription_price_usdc: "free" })).toThrow(
      JsonbContractError
    );
    expect(classifyCreator({ subscriptionPrice: true }).classification).toBe(
      "invalid"
    );
  });
