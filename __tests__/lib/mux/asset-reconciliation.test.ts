...
describe("runMuxReconciliation", () => {
  ...
  it("never treats a failed listing as missing assets", async () => {
    ...
    const { status, metrics, detail } = await runMuxReconciliation(ctx());

    expect(status).toBe("partial");
    expect(metrics.listing_complete).toBe(0);
    expect(detail?.listing_error).toMatch(/Too Many Requests/);
    expect(metrics.direction_b_skipped).toBe(1);
    expect(retrieve).not.toHaveBeenCalled();
    expect(findingInserts()).toEqual([]);
  });
  ...
  it("records a failed check instead of a missing asset when Mux errors", async () => {
    ...
    expect(detail?.check_failed_asset_ids).toEqual(["unknown"]);
    expect(findingInserts()).toEqual([]);
  });
