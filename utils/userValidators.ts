...
export function validateSocialLink(socialLink: any): boolean {
  if (!socialLink || typeof socialLink !== "object") {
    return false;
  }

  // Check if url exists and is a string
  if (!socialLink.url || typeof socialLink.url !== "string") {
    return false;
  }

  // Check if title exists and is a string
  if (!socialLink.title || typeof socialLink.title !== "string") {
    return false;
  }
...
export function validateUserRegistration(input: UserRegistrationInput): {
  ...
  if (!input.wallet) {
    return { isValid: false, error: "Wallet is required" };
  }
