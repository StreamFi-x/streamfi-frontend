...
function makePost(body: unknown): NextRequest {
  return new NextRequest(
    "http://localhost/api/routes-f/stream-collab-invite-accept",
    {
      method: "POST",
...
describe("POST /api/routes-f/stream-collab-invite-accept", () => {
  it("accepts a pending invite and returns a collab session id", async () => {
    ...
    expect(typeof body.collab_session_id).toBe("string");
    expect(body.collab_session_id.length).toBeGreaterThan(0);
  });
