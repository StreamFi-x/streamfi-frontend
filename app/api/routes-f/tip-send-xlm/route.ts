...
/**
 * POST /api/routes-f/tip-send-xlm
 * 
 * Build and return an unsigned XLM payment transaction for tipping.
 * 
 * Request body:
 * {
...
const tipSendXlmSchema = z.object({
  amount: z
    .string()
...
type TipSendXlmRequest = z.infer<typeof tipSendXlmSchema>;
...
async function getRecipientWallet(channelId: string): Promise<string | null> {
  try {
    ...
    console.error("[tip-send-xlm] Error fetching recipient wallet:", error);
    throw error;
  }
}
...
async function getSenderWallet(userId: string): Promise<string | null> {
  try {
    ...
    console.error("[tip-send-xlm] Error fetching sender wallet:", error);
    throw error;
  }
}
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const bodyResult = await validateBody(req, tipSendXlmSchema);
  ...
  try {
    ...
    const transaction = await buildTipTransaction({
      ...
      amount: formatXLMAmount(amountNum),
      network,
    });
    ...
    console.error("[tip-send-xlm] Error building transaction:", error);
