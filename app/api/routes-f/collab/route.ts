...
export async function POST(req: NextRequest): Promise<NextResponse> {
    ...
    `;
        ...
        if (receiverId === session.userId) {
            return NextResponse.json({ error: "You cannot collab with yourself" }, { status: 400 });
        }

        // 2. Validate proposed_date is in the future
        if (proposed_date) {
            if (new Date(proposed_date) <= new Date()) {
                return NextResponse.json({ error: "proposed_date must be in the future" }, { status: 400 });
            }
        }
    ...
    `;
        if (pendingRows[0].count >= 5) {
            return NextResponse.json({ error: "Maximum of 5 pending requests reached" }, { status: 429 });
        }
    ...
    `;
        if (existingRows.length > 0) {
            return NextResponse.json({ error: "A pending request already exists between you" }, { status: 409 });
        }
