...
function makeReq(query: string) {
  return new globalThis.Request(`http://localhost/api/routesF/emoji-picker?${query}`);
}
...
describe('/api/routesF/emoji-picker', () => {
  it('returns emojis filtered by category', async () => {
    ...
    expect(data.emojis.every((item: unknown) => item?.category === 'animals')).toBe(true);
  ...
  it('allows category any and returns mixed category emojis', async () => {
    ...
    const categories = Array.from(new Set(data.emojis.map((item: unknown) => item?.category)));
    expect(categories.length).toBeGreaterThanOrEqual(1);
    expect(categories.every((category) => ['faces', 'animals', 'food'].includes(category))).toBe(true);
  });
