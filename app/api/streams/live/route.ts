...
export async function GET(req: Request) {
  try {
    ...
    if (viewerWallet) {
      ...
      if (viewerResult.rows.length > 0 && Array.isArray(viewerResult.rows[0].following)) {
        viewerFollowing = viewerResult.rows[0].following;
      }
    }

    // Map streams with structured output
    ...
    // Sort streams: followed first, then by viewer count descending
    streams.sort((a, b) => {
    ...
    console.error("[routes-f] Error fetching live streams:", error);
