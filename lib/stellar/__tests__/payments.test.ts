...
describe("Stellar Payments", () => {
  ...
  describe("buildTipTransaction", () => {
    const validSender = "GXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX";
    const validRecipient = "GYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYY";
    ...
    it("should reject invalid amount (negative)", async () => {
      await expect(
        buildTipTransaction(validSender, validRecipient, "-5")
      ).rejects.toThrow("Invalid amount");
    });

    it("should reject invalid amount (zero)", async () => {
      await expect(
        buildTipTransaction(validSender, validRecipient, "0")
      ).rejects.toThrow("Invalid amount");
    });

    it("should reject invalid amount (NaN)", async () => {
      await expect(
        buildTipTransaction(validSender, validRecipient, "abc")
      ).rejects.toThrow("Invalid amount");
    });

    it("should handle account not found error", async () => {
      const mockServer = new StellarSdk.Horizon.Server("https://horizon-testnet.stellar.org");
      (mockServer.loadAccount as jest.Mock).mockRejectedValue(
        new Error("Account not found")
      );

      await expect(
        buildTipTransaction(validSender, validRecipient, "10")
      ).rejects.toThrow();
    });
  ...
  describe("submitTransaction", () => {
    const mockXDR = "AAAAAF2V..."; // Mock XDR string
    ...
    it("should submit transaction successfully", async () => {
      ...
      const mockKit = new StellarWalletsKit({});
      (mockKit.signTransaction as jest.Mock).mockResolvedValue({
        signedTxXdr: mockXDR,
      });

      const mockServer = new StellarSdk.Horizon.Server("https://horizon-testnet.stellar.org");
      (mockServer.submitTransaction as jest.Mock).mockResolvedValue({
        hash: "ABC123",
        successful: true,
      });

      const result = await submitTransaction(mockXDR, mockPublicKey);
    ...
    it("should handle transaction submission failure", async () => {
      ...
      const mockServer = new StellarSdk.Horizon.Server("https://horizon-testnet.stellar.org");
      (mockServer.submitTransaction as jest.Mock).mockRejectedValue(
        new Error("Transaction failed")
      );

      await expect(
        submitTransaction(mockXDR, mockPublicKey)
      ).rejects.toThrow();
    });
    ...
    it("should handle user rejection", async () => {
      const { StellarWalletsKit } = require("@creit.tech/stellar-wallets-kit");
      const mockKit = new StellarWalletsKit({});
      (mockKit.signTransaction as jest.Mock).mockRejectedValue(
  ...
  describe("hasInsufficientBalance", () => {
    ...
    it("should return false when balance is sufficient", async () => {
      ...
      (mockServer.call as jest.Mock).mockResolvedValue({
  ...
  describe("getAccount", () => {
    ...
    it("should load account successfully", async () => {
      ...
      const mockServer = new StellarSdk.Horizon.Server("https://horizon-testnet.stellar.org");
      (mockServer.loadAccount as jest.Mock).mockResolvedValue(mockAccount);

      const account = await getAccount(mockPublicKey);

      expect(account).toBeDefined();
      expect(mockServer.loadAccount).toHaveBeenCalledWith(mockPublicKey);
    });
  ...
  describe("getMinimumBalance", () => {
    ...
    it("should return default value on error", async () => {
      ...
      (mockServer.order as jest.Mock).mockReturnThis();
      (mockServer.limit as jest.Mock).mockReturnThis();
      (mockServer.call as jest.Mock).mockRejectedValue(new Error("API error"));
