...
import type { JobMetrics, JobOutcome } from "@/lib/jobs/scheduled-job";

/** Per-run context supplied by the cron route (see runScheduledJob). */
...
export interface MuxSweepDetail {
  listing_error: string | null;
  check_failed_asset_ids: string[];
}
...
export async function runMuxReconciliation(
  ctx: SweepContext
): Promise<JobOutcome<MuxSweepDetail>> {
  ...
  const { listing_error, check_failed_asset_ids, ...rest } = metrics;
  const numeric: JobMetrics = {};
  for (const [key, value] of Object.entries(rest)) {
    numeric[key] = typeof value === "boolean" ? Number(value) : value;
  }
  const alerts: string[] = [];
  if (metrics.new_findings > 0) {
    alerts.push(
      `${metrics.new_findings} new Mux drift finding(s); review /api/admin/reconciliation/mux`
    );
  }
  return {
    ...
    metrics: numeric,
    alerts,
    detail: { listing_error, check_failed_asset_ids },
  };
}
