 ...
 *
 * Identity: a recording row and a Mux asset correspond when
 * stream_recordings.mux_asset_id = asset.id (UNIQUE in the table). Playback
 * IDs are carried for investigation only — an asset can have several and they
...
async function findRecordedAssetIds(ids: string[]): Promise<Set<string>> {
  ...
  for (let i = 0; i < ids.length; i += DB_BATCH) {
    ...
    const { rows } = await sql`
      ...
      WHERE mux_asset_id IN (
        SELECT jsonb_array_elements_text(${JSON.stringify(chunk)}::jsonb)
      )
    `;
...
async function reconcileMuxToDb(
  ...
  const { rowCount } = await sql`
    ...
    WHERE f.kind = 'MUX_ASSET_WITHOUT_DB_ROW'
      AND f.status = 'open'
      AND EXISTS (SELECT 1 FROM stream_recordings r WHERE r.mux_asset_id = f.mux_asset_id)
  `;
...
async function reconcileDbToMux(
  ...
  for (;;) {
    ...
    const { rows } = await sql`
      SELECT id, mux_asset_id, playback_id, user_id, status, created_at
    ...
    for (const row of rows) {
      ...
      metrics.auto_hidden += await autoHideIfConfirmed(assetId, dbNow);
    }
    await ctx.renewLease();
  }
  ...
  const { rows: open } = await sql`
    SELECT f.id, f.mux_asset_id, (r.id IS NULL) AS row_gone
    FROM mux_drift_findings f
    LEFT JOIN stream_recordings r ON r.id = f.recording_id
...
/**
 * Hide a recording whose asset has been confirmed missing on two runs at least
...
async function autoHideIfConfirmed(
  ...
  const confirmedBefore = new Date(dbNow - AUTO_HIDE_MIN_SPAN_MS);
  const { rows } = await sql`
    WITH finding AS (
      SELECT id, recording_id FROM mux_drift_findings
      WHERE kind = 'DB_ROW_WITHOUT_MUX_ASSET'
        AND status = 'open'
        AND mux_asset_id = ${assetId}
        AND detection_count >= 2
        AND first_detected_at <= ${confirmedBefore.toISOString()}::timestamptz
      FOR UPDATE
    ),
    hidden AS (
      ...
      FROM finding
      WHERE r.id = finding.recording_id
...
async function upsertFinding(input: FindingInput): Promise<boolean> {
  const { rows } = await sql`
    INSERT INTO mux_drift_findings (
      kind, mux_asset_id, recording_id, playback_id, user_id,
      mux_live_stream_id, asset_created_at, previous_status,
      first_detected_run_id, last_detected_run_id
    )
    VALUES (
      ${input.kind}, ${input.muxAssetId}, ${input.recordingId}, ${input.playbackId},
  ...
  if (action === "restore") {
    if (
      ...
      return {
        ok: false,
        status: 409,
        error: "Only a hidden recording can be restored",
      };
    }
    ...
    const { rowCount } = await sql`
      ...
      SET status = ${finding.previous_status ?? "ready"}, unavailable_at = NULL
      WHERE id = ${finding.recording_id} AND status = 'unavailable'
    `;
    ...
    return rowCount === 1
      ? { ok: true, action }
      : { ok: false, status: 409, error: "Recording is no longer hidden" };
  }
    ...
    case "mark_unavailable": {
      if (isOrphanAsset) {
        return {
          ok: false,
          status: 409,
          error: "Only recordings can be marked unavailable",
        };
      }
      if (await retrieveMuxAsset(assetId)) {
        await closeFinding(findingId, "resolved", "none", actor);
        return {
          ok: false,
          status: 409,
          error: "The Mux asset exists; recording left visible",
        };
      }
      await sql`
        UPDATE stream_recordings
        SET status = 'unavailable', unavailable_at = now()
        WHERE id = ${finding.recording_id}
          AND mux_asset_id = ${assetId}
