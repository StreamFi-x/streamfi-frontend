 ...
 * structural invariant (see db/migrations/20260925_02_jsonb_contract_functions.sql)
...
/** Legacy element shapes: types/user.ts SocialLink and utils/userValidators. */
  ...
  for (const link of links) {
    const [key, url] =
      "socialLink" in link
        ...
        : [link.platform.trim().toLowerCase(), link.url];
    if (key in map) {
      return null;
    }
    map[key] = url;
 ...
 *               quarantined before migration 20260925_03 can be applied
 */
