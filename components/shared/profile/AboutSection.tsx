    ...
    <div className={combineClasses(componentClasses.card, "p-6 mb-6")}>
      <div className="flex items-center justify-between mb-4">
        <div className="flex flex-col items-center gap-6">
          <h2 className={combineClasses(textClasses.primary, "text-lg font-medium")}>About {username}</h2>
          <span className={combineClasses(textClasses.secondary, "text-sm mr-4")}>
            <span className={textClasses.highlight}>{followers.toLocaleString()}</span> followers
          </span>
        </div>
        ...
        <div className="flex items-center">
          {isOwner && (
            <Button
              variant="outline"
              size="sm"
              className={combineClasses(buttonClasses.outline, textClasses.primary, "border-none")}
        ...
        <div className="flex space-x-4 mb-4">
          {socialLinks.twitter && (
            <a
              ...
              className={combineClasses(textClasses.tertiary, `hover:${textClasses.primary}`)}
            >
      ...
      {userBio ? (
        <p className={combineClasses(textClasses.secondary, "text-sm whitespace-pre-line")}>{userBio}</p>
      ) : (
        <div className={combineClasses(textClasses.tertiary, "text-sm")}>
          {isOwner ? (
            <div className={combineClasses(bgClasses.tertiary, "p-4 rounded-md")}>
              <p className="mb-2">You haven&apos;t added a bio yet.</p>
              <p>Tell viewers about yourself, your content, and your streaming schedule.</p>
              <Button
                variant="outline"
                size="sm"
                className={combineClasses(buttonClasses.secondary, textClasses.onColor, "border-none mt-3")}
                onClick={() => setShowBioModal(true)}
              >
                Add Bio
              </Button>
