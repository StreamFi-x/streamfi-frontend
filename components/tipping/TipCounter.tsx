...
export const TipCounterSkeleton = ({
  variant,
}: {
  variant: "compact" | "default" | "large";
}) => {
    ...
    >
      <Gift
        className={cn(
          ...
          variant === "compact" ? "w-4 h-4 mb-1" : "w-10 h-10 mb-3"
        )}
      />
      <p
        className={cn(
...
export function TipCounter({
  ...
  const {
    ...
    isLoading,
    mutate: revalidate,
  } = useSWR(`/api/users/${username}/stats`, fetcher, {
  ...
  if (isLoading) {
    return <TipCounterSkeleton variant={variant} />;
  }
  if (error) {
    return (
      <TipCounterError
  ...
  const isZero =
    parseInt(stats.totalCount.toString()) === 0 &&
    parseFloat(stats.totalReceived) === 0;
  ...
  const usdValue = xlmPrice
    ? (parseFloat(stats.totalReceived) * xlmPrice).toLocaleString("en-US", {
        style: "currency",
        currency: "USD",
      })
    : null;

  const lastTipReceived =
    stats.lastTipAt && isMounted
        ...
        <div className="flex items-center gap-2">
          <div
            className={cn(
              "p-1.5 rounded-full",
              hasReachedMilestone
                ? "bg-highlight/20 text-highlight shadow-[0_0_10px_rgba(255,200,0,0.2)]"
          ...
          <div>
            <div className="text-[11px] font-bold text-foreground flex items-center gap-1">
              ...
              {hasReachedMilestone && (
                <TrendingUp className="w-2 h-2 text-highlight animate-pulse" />
              )}
            </div>
      ...
      >
        ...
        <div
          className={cn(
            ...
            hasReachedMilestone
              ? "bg-highlight/20 opacity-60 animate-pulse"
              : "bg-highlight/5 group-hover:bg-highlight/10"
          )}
        />
        ...
        <div className="relative z-10">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Total tips received via Stellar
                </p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
            <div className="space-y-1">
              ...
              {isZero && (
                <div className="mt-2 p-3 border border-dashed border-highlight/30 rounded-xl bg-highlight/5 max-w-xs">
                  <p className="text-[10px] text-highlight/80 font-semibold mb-1 uppercase tracking-wider">
                    💡 Tip for Success
                  </p>
                  <p className="text-xs text-muted-foreground leading-relaxed">
                    Your earnings are currently at zero.{" "}
                    <span className="text-foreground font-medium underline cursor-pointer hover:text-highlight transition-colors">
                      Setup your tip link
                    </span>{" "}
                    to start receiving XLM.
                  </p>
                </div>
              )}
              ...
              <div className="flex items-center gap-2 h-6 mt-1">
                <AnimatePresence mode="wait">
                  {usdValue ? (
                    ...
                    <div
                      key="placeholder"
                      className="w-24 h-4 bg-muted animate-pulse rounded"
                    />
                  )}
                </AnimatePresence>
              </div>
            ...
            <div className="bg-background/40 backdrop-blur-md rounded-2xl p-5 border border-border/50 flex items-center justify-between">
              <div>
                <p className="text-xs uppercase tracking-wider font-bold text-muted-foreground/60 mb-1">
                  Total Contributions
                </p>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 pt-6 border-t border-border/50">
            <div className="flex items-center gap-2 text-xs text-muted-foreground font-medium">
              ...
              {lastTipReceived
                ? `Last tip received ${lastTipReceived}`
                : "No recent tips"}
            </div>

            <button
              onClick={() =>
                handleCopy(walletAddressOverride ?? stats.stellarPublicKey)
              }
            ...
            >
              <span className="text-xs font-mono text-muted-foreground group-hover/copy:text-foreground">
                {(walletAddressOverride ?? stats.stellarPublicKey).slice(0, 8)}
                ...{(walletAddressOverride ?? stats.stellarPublicKey).slice(-8)}
              </span>
              {copied ? (
        ...
        <div className="h-5">
          <AnimatePresence mode="wait">
            {usdValue ? (
              ...
              <div
                key="placeholder"
                className="w-16 h-3 bg-muted animate-pulse rounded mt-1"
              />
            )}
          </AnimatePresence>
        </div>
