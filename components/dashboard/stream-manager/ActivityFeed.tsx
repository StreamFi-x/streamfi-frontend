    ...
    <div className="h-full bg-card border border-border rounded-xl overflow-hidden flex flex-col">
      <div className="px-4 py-2.5 border-b border-border flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-foreground">Stream Activity</span>
        </div>
        {isLive && (
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-hide">
        ...
        <div className="grid grid-cols-3 gap-2">
          <SessionStat icon={<Timer className="w-3.5 h-3.5" />} label="Session" value={sessionTime} mono />
          <SessionStat icon={<Users className="w-3.5 h-3.5" />} label="Viewers" value={viewerCount.toString()} />
          <SessionStat
...
function SessionStat({
  ...
  return (
    <div className="bg-secondary rounded-lg p-2.5 flex flex-col items-center text-center gap-1">
      ...
      <p className={cn("text-xs font-bold text-foreground leading-none", mono && "font-mono", valueClass)}>
        {value}
      </p>
