...
import type {
  ...
  SessionRetentionPoint,
  ChatEngagementPoint,
} from "@/app/api/routes-f/analytics-session-detail/route";
...
function SessionDetailView({ data }: SessionDetailViewProps) {
  ...
  const retentionChartData = retention_curve.map((point: SessionRetentionPoint) => ({
  ...
  const chatChartData = chat_engagement.map((point: ChatEngagementPoint) => ({
    minutes: Math.floor(point.bucket_seconds / 60),
