...
export default function StreamTokenUtility() {
  ...
  return (
    <section className="w-full ">
      <div className="relative grid grid-cols-1 md:grid-cols-2 gap-2.5 sm:gap-8 py-16 md:py-20 px-6 container mx-auto bg-transparent overflow-hidden">
        {/* Content Container */}
        <div className="col-span-1 md:col-span-2 flex flex-col items-center gap-6 relative z-40">
          ...
          <motion.p
            className=" text-sm sm:text-base text-white/80 font-normal sm:text-center max-w-  xl"
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true }}
            variants={fadeInUp}
          ...
          >
            Lorem ipsum dolor sit amet consectetur. Dictum elementum malesuada
            sed a. Cursus sem pellentesque porttitor fringilla consectetur
            egestas
          </motion.p>
        </div>
        ...
        >
          {tokenUtilityData.map((item, index) => (
            <motion.div
              key={index}
              className="flex z-50 items-start gap-4 p-4 xl:p-6 bg-[#FFFFFF0D] border border-[#FFFFFF1A] rounded-lg hover:translate-y-[-8px] transition-transform duration-300 cursor-pointer"
              variants={fadeInUp}
