...
| --------------- | -------------------------------------------------------------------------------- | --------------------------------------------------------- | ---------------------------------------------------- |
| Edge (HTTP/CDN) | `Cache-Control: public, s-maxage=…` on route responses, honoured by Vercel's CDN | TTL only. Application code cannot purge one key.          | Public data where a few seconds of staleness is fine |
...
| `referenceData`    | `public, s-maxage=60, stale-while-revalidate=300`       | 1h Redis                        | Category create/update/delete                     | App layer exact; edge adds ≤6min                                                        |
...
| `privateAnalytics` | `private, max-age=60`                                   | none                            | TTL                                               | Browser-only, per user                                                                  |
...
| ----------------------------------------------------- | ------------------------------- | --------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| `routes-f/creator/analytics`                          | `public, s-maxage=300, swr=300` | `private, max-age=60` | Identity comes from the session cookie and the URL is the same for every creator. The CDN could serve one creator's revenue to another. |
| `routes-f/analytics-session-list`                     | `public, s-maxage=60, swr=300`  | `private, max-age=60` | Session-authenticated, same reason                                                                                                      |
| `routes-f/analytics-session-detail`                   | `public, s-maxage=300, swr=600` | `private, max-age=60` | Session-authenticated, same reason                                                                                                      |
