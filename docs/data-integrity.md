...
3. [Mux asset ↔ `stream_recordings` consistency sweep](#3-mux-asset--stream_recordings-sweep-1409) (#1409)
...
| `sociallinks`   | `[{ platform, url, title? }]`                                        | normalised to the map (platform lower-cased)                                                                                  |
...
stream and playback lookups, recordings/clips, chat history, sitemap, schedule
reminders and notifications. Their content is hidden, not deleted, until the
...
`users` neither filters `deleted_at` nor carries a `-- tombstone-aware: <reason>`
comment (or lives in a file on the internal allowlist in that test).
...
`GET /api/cron/purge-deleted-users` (daily). For each due, unheld deletion
...
2. `mux_assets` — delete the Mux assets of the user's recordings (404 = done)
...
| -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| delete   | `stream_sessions.user_id` (cascades to its chat, viewers), `chat_messages.user_id`, `user_follows.*`, `stream_recordings.user_id`, `stream_schedule.creator_id`, `stream_reminders.viewer_id`, `user_badges.user_id`, `stream_tags.stream_id`, `tag_suggestions.suggested_by`, `channel_emotes.creator_id`, `mock_*_transactions.creator_id` |
| set NULL | `chat_messages.moderated_by`, `stream_viewers.user_id`, `moderation_queue.reporter_id/reported_user_id/assigned_to`, `moderation_audit_log.moderator_id`, `mock_*_transactions.viewer_id`                                                                                                                                                    |
| preserve | `tip_transactions.*`, `gift_transactions.*`, `subscriptions.*`, `subscription_tiers.creator_id`, `payouts.user_id`, `user_deletions.user_id`                                                                                                                                                                                                 |
...
## 3. Mux asset ↔ `stream_recordings` sweep (#1409)

On this branch `stream_recordings` is the only table that references Mux
assets; the "clips" pages are backed by it. A row and an asset correspond when
`stream_recordings.mux_asset_id = asset.id` (unique). Playback ids are carried
for investigation only.

`GET /api/cron/mux-reconciliation` (daily):
...
3. **DB row without a Mux asset** — only runs when the listing was complete.
...
| DB row without Mux asset | after **two** 404 confirmations at least 24 hours apart, the recording is hidden (`status = 'unavailable'`, `unavailable_at`); the row and its previous status are kept. Resolved automatically if the asset reappears or the row is deleted | `mark_unavailable` (hide early, after re-checking 404), `restore` (only once the asset exists again), `dismiss`                         |

Every public read already filters `status = 'ready'`, so hidden recordings stop
appearing in the UI.

Related changes that reduce drift at the source: the webhook now returns `500`
when saving the recording fails, so Mux redelivers the event instead of the
asset being orphaned; and when an owner deletes a recording, its Mux asset is
deleted too (a failure there is left for the sweep to report).
...
one to observe. `GET /api/cron/tip-reconciliation` (hourly) compares, for
...
payment filter as `/api/tips/refresh-total` — with the stored rows, by
...
`/api/tips/refresh-total` had `ON CONFLICT (tx_hash)`, which cannot use the
partial unique index on `tx_hash` and made every insert fail; it now uses
`ON CONFLICT (tx_hash) WHERE tx_hash IS NOT NULL` and sums amounts exactly.
...
| ------------------ | ------------------------------- | ------------------------ |
| Tip reconciliation | `/api/cron/tip-reconciliation`  | hourly at :15            |
| Mux sweep          | `/api/cron/mux-reconciliation`  | daily 03:30 UTC          |
| Account purge      | `/api/cron/purge-deleted-users` | daily 04:00 UTC          |
