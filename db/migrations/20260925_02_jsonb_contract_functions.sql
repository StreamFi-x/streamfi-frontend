...
--                 sociallinks shape). Unknown keys are left to the app layer.
...
-- constraints are added by 20260925_03 after the audit has run.
...
BEGIN
  ...
  FOR kv IN SELECT * FROM jsonb_each(v) LOOP
    IF kv.key IN ('streamTitle', 'title', 'description', 'category', 'payout', 'thumbnail', 'lastUpdated') THEN
      IF jsonb_typeof(kv.value) NOT IN ('string', 'null') THEN
        RETURN false;
      END IF;
